...
function categoryUrl(line) {
  ...
  return CATEGORY_INFO && CATEGORY_INFO[key] ? `${SITE_URL}/category/${key}` : `${SITE_URL}/products`;
}
...
const STATIC_PAGES = {
  "/about": {
    ...
    body: `<p>NIX-BUILDS یک برند طراحی و ساخت است که روی محصولات کاربردی، مدرن و قابل تولید با فناوری‌های ساخت دیجیتال تمرکز دارد.</p><h2>لاین‌های NIX-BUILDS</h2><div class="route-links"><a class="route-link" href="/category/nix-home"><strong>NIX-HOME</strong><br>محصولات خانه و دکور</a><a class="route-link" href="/category/nix-gear"><strong>NIX-GEAR</strong><br>ابزار و تجهیزات کاربردی</a><a class="route-link" href="/category/nix-care"><strong>NIX-CARE</strong><br>محصولات مراقبت و زندگی روزمره</a></div><h2>طراحی با تمرکز بر کاربرد</h2><p>هدف ما ترکیب فرم، عملکرد و ساخت دیجیتال برای ارائه محصولاتی است که هم ظاهر حرفه‌ای داشته باشند و هم در استفاده روزمره مفید باشند.</p>`
  },
  "/contact": {
    title: "تماس و سفارش | NIX-BUILDS",
    description: "راه‌های تماس و ثبت سفارش محصولات NIX-BUILDS.",
    h1: "تماس و سفارش",
    body: `<p>برای پرسش درباره محصول، رنگ، مشخصات، قیمت یا ثبت سفارش می‌توانید از اطلاعات تماس زیر استفاده کنید.</p><div class="route-links"><a class="route-link" href="tel:+989030287529">📞 تلفن: 00989030287529</a><a class="route-link" href="mailto:kaveh-m@live.com">✉️ ایمیل: kaveh-m@live.com</a><a class="route-link" href="/products">🛍️ مشاهده محصولات</a></div><h2>ثبت سفارش</h2><p>در صفحه محصول، کالا را به سبد اضافه کنید و از بخش سبد خرید، درخواست سفارش را ارسال کنید. در موبایل، متن سفارش در برنامه پیامک آماده می‌شود و ارسال نهایی با تأیید شما انجام خواهد شد.</p>`
  },
  "/shipping": {
...
function routeShell(source, meta, bodyHtml, canonicalPath, schemaExtra = "") {
  ...
  const content = `<main class="route-content" data-route-path="${escHtml(canonicalPath)}"><div class="route-card"><div class="product-breadcrumbs"><a href="/" data-internal-route="/">NIX-BUILDS</a> / <span id="routeBreadcrumbTitle">${escHtml(meta.h1)}</span></div><h1 id="routeTitle">${escHtml(meta.h1)}</h1><div id="routeBody">${bodyHtml}</div></div></main>`;
...
async function sitemapResponse() {
  ...
  const urls = [
    ...
    `<url><loc>${SITE_URL}/products</loc></url>`,
    `<url><loc>${SITE_URL}/category/nix-home</loc></url>`,
    `<url><loc>${SITE_URL}/category/nix-gear</loc></url>`,
    `<url><loc>${SITE_URL}/category/nix-care</loc></url>`,
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (match) {
      ...
      if (!product) {
        const fallback = await env.ASSETS.fetch(new Request(new URL("/index.html", url), { method:"GET", headers:request.headers }));
        if (fallback.ok) return new Response(await fallback.text(), {status:200, headers:{"content-type":"text/html; charset=UTF-8","cache-control":"no-store"}});
        return new Response("Product not found", { status: 404, headers: {"content-type":"text/plain; charset=UTF-8"} });
      }
      ...
      return new Response(rendered, {status:200, headers:{"content-type":"text/html; charset=UTF-8","cache-control":"public, max-age=60, s-maxage=300","vary":"Accept-Encoding"}});
    }
    ...
    if (sourceAsset.ok) {
      try {
        ...
        if (staticResponse) return new Response(staticResponse, {status:200, headers:{"content-type":"text/html; charset=UTF-8","cache-control":"public, max-age=300, s-maxage=1800"}});
      } catch (error) {
