...
function routeShell(source, meta, bodyHtml, canonicalPath, schemaExtra = "") {
  ...
  const content = `<main class="route-content"><div class="route-card"><div class="product-breadcrumbs"><a href="/">NIX-BUILDS</a> / ${escHtml(meta.h1)}</div><h1>${escHtml(meta.h1)}</h1>${bodyHtml}</div></main>`;
  out = out.replace('<section id="serverProductSEO" class="server-product-seo" aria-label="Product information" style="display:none;"></section>', content);
  return out;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    const sourceAsset = await env.ASSETS.fetch(new Request(new URL("/index.html", url), request));
    if (sourceAsset.ok) {
      const staticResponse = await staticRouteResponse(url.pathname.replace(/\/$/, "") || "/", await sourceAsset.clone().text());
    ...
    if (match) {
      ...
      const rendered = injectProductSEO(source, product, related);
